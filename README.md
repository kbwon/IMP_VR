...
> 플레이어가 폐건물/저택을 탐사하며 특수 카메라로 보이지 않는 단서와 위험을 발견하고, 방마다 배치된 퍼즐과 추격 이벤트를 해결해 탈출하는 VR 공포 퍼즐 게임입니다.
...
이 프로젝트는 게임 회사 포트폴리오 제출 시 다음 역량을 보여주기 위한 VR 게임 프로젝트입니다.
...
플레이어는 폐건물 내부의 방을 순서대로 탐색하면서 다음 흐름을 반복합니다.
...
프로젝트는 팀원별 작업 장면과 스크립트를 나누어 개발한 뒤 통합하는 방식으로 진행했습니다.
